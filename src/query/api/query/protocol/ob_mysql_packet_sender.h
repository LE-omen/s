...
#ifndef OCEANBASE_QUERY_MYSQL_PACKET_SENDER_H_
#define OCEANBASE_QUERY_MYSQL_PACKET_SENDER_H_
...
class ObIMPPacketSender : public sql::ObIClientPacketChannel
{
public:
  ...
  virtual int wait_packet(obmysql::ObICSMemPool& mem_pool,
                          int64_t timeout_us,
                          obmysql::ObMySQLPacket *&pkt) override = 0;
  virtual int release_packet(obmysql::ObMySQLPacket* pkt) override = 0;
  virtual int response_packet(obmysql::ObMySQLPacket &pkt) override = 0;
  virtual int response_resultset_metadata(
      ...
      uint16_t status_flags) override = 0;
  ...
  virtual int flush_buffer(const bool is_last) override = 0;
...
#endif /* OCEANBASE_QUERY_MYSQL_PACKET_SENDER_H_ */
