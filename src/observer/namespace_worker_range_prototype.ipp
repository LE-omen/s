...
int process_ranges(uint64_t ns, Frame &request, Frame &reply) {
  ...
  if (!ret && (ns != 1 || !tablet.is_valid() || !count || count > MAX_FRAME / 32
  ...
  for (uint64_t i = 0; !ret && i < count; ++i) {
    ...
    if (!ret && !owns_table(ns, range.get_table_id())) { ret = OB_INVALID_ARGUMENT; }
  ...
  if (!ret && operation == 'C') { reply.number(size); }
    ...
    for (int64_t i = 0; i < split.count(); ++i) {
      ...
      for (int64_t j = 0; j < split.count(i); ++j) { append_range(reply, split.at(i, j)); }
    }
  }
  return reply.ret;
}
