...
int ObTransformUtils::merge_from_items_as_inner_join(ObTransformerCtx *ctx,
                                                     ObDMLStmt &stmt,
                                                     ...
                                                     TableItem *&ret_table)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < from_item_list.count(); ++i) {
    const FromItem &item = from_item_list.at(i);
    TableItem *table_item = NULL;
    if (item.is_joined_) {
      JoinedTable *joined_table = NULL;
      bool found = false;
      for (int64_t j = 0; !found && j < joined_table_list.count(); ++j) {
        JoinedTable *cur_table = joined_table_list.at(j);
        if (NULL != cur_table) {
          if (cur_table->table_id_ == item.table_id_) {
      ...
      table_item = joined_table;
      if (OB_FAIL(ObOptimizerUtil::remove_item(joined_table_list, joined_table))) {
      }
    } else {
      table_item = stmt.get_table_item_by_id(item.table_id_);
    }
    if (OB_ISNULL(table_item)) {
      ...
      ret_table = table_item;
    } else if (OB_FAIL(add_new_joined_table(ctx, stmt, INNER_JOIN, ret_table,
                                            table_item, dummy_conds, ret_table,
                                            false /* add_table */))) {
    }
  }
                                              ...
                                              ObIArray<ObRawExpr*> &column_exprs)
{
  ...
  if (OB_UNLIKELY(!view_table_item.is_generated_table() &&
      ...
      !view_table_item.is_lateral_table())) {
    ret = OB_ERR_UNEXPECTED;
                                              ...
                                              bool repeated_select) //create select item and new column
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < new_select_list.count(); ++i) {
    ...
    if (OB_ISNULL(expr = new_select_list.at(i))) {
      ret = OB_ERR_UNEXPECTED;
    } else if (OB_FAIL(expr->formalize(ctx->session_info_))) {
    }
  ...
  if (OB_ISNULL(expr)) {
    ...
    if (OB_SUCC(ret) && (expr->get_expr_type() == T_OP_AND ||
      ...
      if (OB_SUCC(ret) && happend) {
        ...
        op_expr->clear_child();
        if (OB_FAIL(op_expr->set_param_exprs(new_param_exprs))) {
        } else if (NULL != trans_happened) {
          *trans_happened = true;
        }
      }
                                                    ...
                                                    ObIArray<ObRawExpr*> &having_exprs_for_deduce)
{
  ...
  if (OB_ISNULL(sel_stmt)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (sel_stmt->get_rollup_expr_size() == 0) {
                                                   ...
                                                   ObSQLSessionInfo *session_info)
{
  ...
  if (OB_ISNULL(from_expr) || OB_ISNULL(to_expr)) {
    ...
    bool need_length_cast = ob_is_string_or_lob_type(dst_type.get_type()) 
...
int ObTransformUtils::is_equal_correlation(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                           ObRawExpr *cond,
                                           bool &is_valid,
                                           ...
                                           ObRawExpr **inner_param)
{
  ...
  if (OB_ISNULL(cond)) {
    ret = OB_ERR_UNEXPECTED;
             ...
             OB_ISNULL(right = cond->get_param_expr(1))) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(ObTransformUtils::is_correlated_expr(exec_params,
                                                          left,
                                                          left_is_correlated))) {
  } else if (OB_FAIL(ObTransformUtils::is_correlated_expr(exec_params,
                                                          right,
...
int ObTransformUtils::check_at_least_one_row(TableItem *table_item, bool &at_least_one_row)
{
  ...
  if (OB_ISNULL(table_item)) {
    ...
    ret = OB_ERR_UNEXPECTED;
             ...
             view_stmt->has_window_function_filter()) {
    // do nothing
                                         ...
                                         ObAliasRefRawExpr *alias_expr)
{
  ...
  if (OB_SUCC(ret) && push_conditions) {
    if (OB_FAIL(classify_where_conds(*stmt, norm_conds, other_conds))) {
    }
  }
                                         ...
                                         ObIArray<OrderItem> *order_items/* = NULL*/)
{
  ...
  } else if (OB_FAIL(stmt->formalize_stmt(ctx->session_info_, false))) {
  }
  if (OB_SUCC(ret) && stmt->is_select_stmt()) {
...
int ObTransformUtils::classify_where_conds(ObDMLStmt &stmt,
                                                ...
                                                bool &is_true)
{
  ...
  if (OB_ISNULL(ctx) || OB_ISNULL(ctx->exec_ctx_) ||
    ...
    } else if (left_obj_value.is_null() || right_obj_value.is_null()) {
      is_true = false;
    } else {
      int res = 0;
...
int ObTransformUtils::build_row_expr(ObRawExprFactory& expr_factory, common::ObIArray<ObRawExpr*>& param_exprs, ObOpRawExpr*& row_expr) {
  ...
  } else if (OB_FAIL(row_expr->init_param_exprs(param_exprs.count()))) {
  } else {
    for (int64_t i = 0; OB_SUCC(ret) && i < param_exprs.count(); i++) {
                                                     ...
                                                     ObIArray<ObRawExpr *> *unique_keys)
{
  ...
  if (OB_SUCC(ret) && !added_unique_keys.empty()) {
    ...
    } else if (OB_FAIL(select_stmt->formalize_stmt(ctx->session_info_, false))) {
    }
  }
                                                 ...
                                                 bool in_add_expr)
{
  ...
  if (OB_ISNULL(expr) || OB_ISNULL(select_stmt) || OB_ISNULL(select_stmt->get_query_ctx())) {
    ...
    if (groupby_exprs.empty() && rollup_exprs.empty() ) {
    } else {
...
int ObTransformUtils::add_compare_int_constraint(ObTransformerCtx *trans_ctx,
                                                 ObRawExpr *expr,
                                                 ...
                                                 int64_t val)
{
  ...
  if (OB_ISNULL(trans_ctx) || OB_ISNULL(expr) || OB_ISNULL(trans_ctx->exec_ctx_)) {
    ...
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_UNLIKELY(T_OP_EQ != op_type
                         && T_OP_LE != op_type
                         && T_OP_LT != op_type
                         && T_OP_GE != op_type
                         && T_OP_GT != op_type
    ...
    ret = OB_ERR_UNEXPECTED;
  } else if (!expr->has_flag(CNT_STATIC_PARAM)) {
    // do nothing
  } else {
    // construct expr >= 0;
    ObConstRawExpr *int_expr = NULL;
    ObOpRawExpr *true_expr = NULL;
    if (OB_FAIL(expr_factory->create_raw_expr(T_INT, int_expr))) {
    } else if (OB_ISNULL(int_expr)) {
      ret = OB_ERR_UNEXPECTED;
    } else if (OB_FAIL(expr_factory->create_raw_expr(op_type, true_expr))) {
    } else if (OB_ISNULL(true_expr)) {
      ret = OB_ERR_UNEXPECTED;
    } else if (OB_FAIL(true_expr->set_param_exprs(expr, int_expr))) {
    } else {
      ObObj obj;
      obj.set_int(ObIntType, val);
      int_expr->set_value(obj);
    }
 ...
 * */
int ObTransformUtils::check_correlated_exprs_can_pullup(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                                        ...
                                                        bool &can_pullup)
{
  ...
  if (subquery.is_set_stmt()) {
    if (OB_FAIL(check_correlated_exprs_can_pullup_for_set(exec_params, subquery, can_pullup))) {
    }
  } else {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && can_pullup && i < check_exprs.count(); ++i) {
      ...
      if (OB_ISNULL(check_exprs.at(i))) {
        ret = OB_ERR_UNEXPECTED;
      } else if (OB_FAIL(is_correlated_expr(exec_params,
                                            check_exprs.at(i),
                                            is_correlated))) {
      } else if (is_correlated) {
        can_pullup = false;
...
/**
  ...
  * */
int ObTransformUtils::check_correlated_exprs_can_pullup_for_set(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                                                const ObSelectStmt &subquery, 
                                                                bool &can_pullup)
{
  int ret = OB_SUCCESS;
  ...
  for (int64_t i = 0; can_pullup && OB_SUCC(ret) && i < set_queries.count(); ++i) {
    ...
    if (OB_ISNULL(right_query)) {
      ...
      can_pullup = false;
    } else if (OB_FAIL(check_correlated_exprs_can_pullup(exec_params, *right_query, can_pullup))) {
    } else if (!can_pullup) {
      //do nothing
    } else if (0 == i) {
                                                            ...
                                                            const bool ignore_select_item)
{
  ...
  if (OB_ISNULL(left_query) || OB_ISNULL(right_query)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FAIL(add_correlated_flag(left_exec_params, exprs))) {
  } else if (OB_FAIL(add_correlated_flag(right_exec_params, exprs))) {
  ...
  if (OB_SUCC(ret) && is_valid) {
    if (OB_FAIL(get_correlated_conditions(left_exec_params,
                                          left_query->get_condition_exprs(), 
                                          left_where_exprs))) {
    } else if (OB_FAIL(get_correlated_conditions(right_exec_params,
...
int ObTransformUtils::check_fixed_expr_correlated(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                                  const ObSelectStmt &stmt,
                                                  bool &is_valid)
{
  int ret = OB_SUCCESS;
  ...
  for (int64_t i = 0; OB_SUCC(ret) && is_valid && i < fixed_exprs.count(); ++i) {
    bool is_correlated = false;
    if (OB_FAIL(is_correlated_expr(exec_params,
                                   fixed_exprs.at(i),
                                   is_correlated))) {
    } else if (is_correlated) {
      is_valid = false;
...
int ObTransformUtils::check_can_pullup_conds(const ObSelectStmt &view, bool &has_special_expr)
{
  ...
  has_special_expr = (has_limit | has_assign);
  return ret;
}
    ...
    const ObIArray<ObExecParamRawExpr *> &exec_params, const ObSelectStmt &subquery, bool &is_correlated)
{
  ...
  if (OB_SUCC(ret) && !is_correlated) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && !is_correlated && i < semi_infos.count(); ++i) {
      if (OB_FAIL(check_semi_conditions_correlated(exec_params,
                                                   semi_infos.at(i),
                                                   is_correlated))) {
      }
    }
...
/**
  * If there are special expressions such as limit, assignment, etc.
                                             ...
                                             bool &is_same)
{
  ...
  if (OB_ISNULL(left_expr) || OB_ISNULL(right_expr)) {
    ...
    const ObRawExprResType &left_type = left_expr->get_result_type();
    const ObRawExprResType &right_type = right_expr->get_result_type();
...
int ObTransformUtils::check_exprs_contain_lob_type(ObIArray<ObRawExpr *> &exprs, bool &has_lob)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && !has_lob && i < col_exprs.count(); i++) {
    ...
    if (OB_ISNULL(col_expr)) {
      ...
      has_lob = ObLongTextType == col_expr->get_result_type().get_type();
    }
  }
  return ret;
}
...
int ObTransformUtils::pullup_correlated_conditions(const ObIArray<ObExecParamRawExpr *> &exec_params, 
                                                   ObIArray<ObRawExpr *> &exprs, 
    ...
    if (OB_ISNULL(tmp_table_item)) {
      ...
      if (OB_FAIL(ctx->schema_checker_->get_table_schema(
        ...
        for (int64_t i = 0; OB_SUCC(ret) && iter != end; ++iter, i++) {
          ...
          } else if (OB_FAIL(ObRawExprUtils::build_column_expr(*ctx->expr_factory_, *col_schema,
                                                  ...
                                                  ObSysFunRawExpr *json_object_expr)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(json_object_expr->set_param_exprs(new_col_arr))) {
    }
  }
  return ret;
}
    ...
    if (OB_ISNULL(table) || OB_ISNULL(upper_stmt)) {
      ...
      if (0 == j) {
        ...
        if (OB_ISNULL(temp_table_query)) {
          ret = OB_ERR_UNEXPECTED;
        } else if (OB_FAIL(temp_table_query->formalize_stmt(ctx->session_info_, false))) {
        ...
        ret = OB_ERR_UNEXPECTED;
                                                ...
                                                *ctx->expr_factory_,
                                                *temp_table_query))) {
      } else if (OB_FAIL(child_stmt->formalize_stmt(ctx->session_info_, false))) {
...
int ObTransformUtils::is_where_subquery_correlated(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                                   const ObSelectStmt &subquery, 
                                                   bool &is_correlated)
{
  int ret = OB_SUCCESS;
                                                  ...
                                                  bool &is_bypass)
{
  ...
  if (OB_ISNULL(expr) || OB_ISNULL(src_expr)) {
    ...
    if (T_FUN_SYS_CAST == op_type || T_FUN_SYS_CONVERT == op_type) {
      ...
      const ObRawExprResType &dst_type = expr->get_result_type();
...
int ObTransformUtils::is_correlated_exprs(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                          ObIArray<ObRawExpr *> &exprs,
                                          bool &bret)
{
  int ret = OB_SUCCESS;
  bret = false;
  for (int64_t i = 0; OB_SUCC(ret) && !bret && i < exprs.count(); ++i) {
    if (OB_ISNULL(exprs.at(i))) {
      ret = OB_ERR_UNEXPECTED;
    } else if (OB_FAIL(is_correlated_expr(exec_params, exprs.at(i), bret))) {
    }
  }
  return ret;
}
...
int ObTransformUtils::is_select_item_correlated(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                                const ObSelectStmt &subquery, 
                                                bool &is_correlated)
{
  int ret = OB_SUCCESS;
  ...
  const ObRawExpr* expr = NULL;
  is_correlated = false;
  for (int64_t i = 0; OB_SUCC(ret) && !is_correlated && i < item_size; ++i) {
    expr = subquery.get_select_item(i).expr_;
    if (OB_ISNULL(expr)) {
      ret = OB_ERR_UNEXPECTED;
    } else if (OB_FAIL(is_correlated_expr(exec_params, expr, is_correlated))) {
    }
  }
  return ret;
}
...
int ObTransformUtils::check_is_basic_aggr_item(const ObSelectStmt &subquery,
                                               bool &is_valid)
{
  int ret = OB_SUCCESS;
  is_valid = true;
...
int ObTransformUtils::deduce_query_values(ObTransformerCtx &ctx,
                                          ObDMLStmt &stmt,
                                          ...
                                          ObIArray<ObRawExpr *> &real_values)
{
  ...
  if (OB_ISNULL(ctx.session_info_) ||
    ...
    replacer.session_ = ctx.session_info_;
    for (int64_t i = 0; OB_SUCC(ret) && i < is_null_prop.count(); ++i) {
      ...
      if (OB_FAIL(real_values.push_back(view_columns.at(i)))) {
        ...
        ret = OB_ERR_UNEXPECTED;
                                                ...
                                                default_expr,
                                                &replacer))) {
      } else if (OB_FAIL(default_expr->formalize(ctx.session_info_))) {
      } else if (OB_FAIL(ObTransformUtils::build_case_when_expr(stmt,
                                                                not_null_expr,
                                                                view_columns.at(i),
                                                                default_expr,
...
int ObTransformUtils::calc_column_repeat_rate(ObTransformerCtx *ctx,
                                              const TableItem *table,
                                              const ObIArray<uint64_t> &column_ids,
                                              double &repeat_rate)
{
  ...
  const ObTableSchema *table_schema = NULL;
  ObGlobalTableStat table_stat;
  double joint_ndv = 1;
  repeat_rate = -1;
  if (OB_ISNULL(ctx) || OB_ISNULL(ctx->schema_checker_)
      || OB_ISNULL(ctx->opt_stat_mgr_) || OB_ISNULL(ctx->session_info_)
      || OB_ISNULL(table)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (!table->is_basic_table()) {
    // do nothing
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < column_ids.count(); ++i) {
      ...
      if (OB_FAIL(ctx->opt_stat_mgr_->get_column_stat(table->ref_id_,
                                                      ...
                                                      column_ids.at(i),
                                                      column_handle))) {
      } else if (OB_ISNULL(column_handle.stat_)) {
        ret = OB_ERR_UNEXPECTED;
      } else if (i == 0) {
        joint_ndv *= column_handle.stat_->get_num_distinct();
      } else {
        joint_ndv *= column_handle.stat_->get_num_distinct() / std::sqrt(2);
      }
    }
    if (OB_SUCC(ret)) {
      repeat_rate = max(table_stat.get_row_count() / joint_ndv, 1.0);
    }
  } 
  return ret;
}
...
int ObTransformUtils::extract_nullable_exprs(
...
int ObTransformUtils::is_orderby_correlated(const ObIArray<ObExecParamRawExpr *> &exec_params,
                                            const ObSelectStmt &subquery,
                                            bool &is_correlated)
{
  int ret = OB_SUCCESS;
  is_correlated = false;
  for (int64_t i = 0; OB_SUCC(ret) && !is_correlated && i < subquery.get_order_item_size(); ++i) {
    const ObRawExpr *expr = NULL;
...
int ObTransformUtils::check_lateral_ref_outer_table(const ObDMLStmt *stmt,
                                                    ...
                                                    bool &is_ref_outer)
{
  ...
  is_ref_outer = false;
  const JoinedTable *join_table = NULL;
  if (OB_ISNULL(stmt) ||
      OB_ISNULL(parent_table_item) ||
      OB_ISNULL(table_item)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_UNLIKELY(!table_item->is_lateral_table())) {
    ret = OB_ERR_UNEXPECTED;
  } else if (!parent_table_item->is_joined_table()) {
    ret = OB_ERR_UNEXPECTED;
  } else if (OB_FALSE_IT(join_table = static_cast<const JoinedTable*>(parent_table_item))) {
    // do nothing
  } else if (OB_ISNULL(join_table->left_table_) ||
             OB_ISNULL(join_table->right_table_)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (join_table->left_table_ == table_item &&
             OB_FAIL(stmt->get_table_rel_ids(*join_table->right_table_,
                                             parent_rel_ids))) {
  } else if (join_table->right_table_ == table_item &&
             OB_FAIL(stmt->get_table_rel_ids(*join_table->left_table_,
                                             parent_rel_ids))) {
  } else {
    for (int64_t i = 0; OB_SUCC(ret) && !is_ref_outer && i < table_item->exec_params_.count(); ++i) {
      ObExecParamRawExpr *exec_param = table_item->exec_params_.at(i);
      ObRawExpr *ref_expr = NULL;
      if (OB_ISNULL(exec_param) ||
          OB_ISNULL(ref_expr = exec_param->get_ref_expr())) {
        ret = OB_ERR_UNEXPECTED;
      } else if (!ref_expr->get_relation_ids().is_subset2(parent_rel_ids)) {
        is_ref_outer = true;
      }
    }
  }
...
bool ObTransformUtils::is_const_null(ObRawExpr &expr)
{
  ...
  if (!expr.is_const_raw_expr()) {
    ...
    bret = const_expr.get_param().is_null();
  }
  return bret;
}
                                                        ...
                                                        bool& is_valid)
{
  ...
  } else if (right_hand->is_shared_reference()) {
    // do nothing
                                                       ...
                                                       bool &is_valid)
{
  int ret = OB_SUCCESS;
  is_valid = true;
  bool has_pushed_param = false;
  bool has_nlj_path = false;
  ObDMLStmt *root_stmt = parent_stmts.empty() ? stmt : parent_stmts.at(parent_stmts.count()-1).stmt_;
  hash::ObHashMap<uint64_t, ObParentDMLStmt, oceanbase::common::hash::NoPthreadDefendMode> parent_map;
  ObSEArray<ObSelectStmt*, 8> dummy;
  ObSEArray<int64_t, 4> dummy_semi_ids;
  bool ref_fake_cte_table = false;
  // 1. current sub stmt tree must not contain dynamic parameters pushed down from ancestor stmts
  ...
  if (OB_FAIL(ret) || !is_valid) {
    //do nothing
  } else if (OB_FAIL(ObTransformUtils::check_ref_fake_cte_table(stmt, ref_fake_cte_table))) {
  ...
  if (OB_FAIL(ret) || !is_valid || !check_nlj_inner_path || !stmt->is_select_stmt()) {
    //do nothing
  } else if (OB_FAIL(parent_map.create(128, "ParentMap"))) {
  ...
  if (OB_SUCC(ret) && nlj_enabled) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < stmt->get_select_item_size(); i++) {
      ...
      if (OB_FAIL(ObTransformUtils::check_select_item_match_index(root_stmt,
        ...
        // do nothing
      } else if (OB_FAIL(sel_idxs.push_back(i))) {
      }
    }
 ...
 */
int ObTransformUtils::check_projected_cols_used_for_join(const ObSelectStmt *stmt,
                                                        const hash::ObHashMap<uint64_t, ObParentDMLStmt, common::hash::NoPthreadDefendMode> &parent_map,
 ...
 */
int ObTransformUtils::can_push_dynamic_filter_to_cols(const ObSelectStmt *stmt,
...
int ObTransformUtils::extract_pushdown_cols(const ObQueryRefRawExpr &query_ref,
...
int ObTransformUtils::collect_semi_join_stmt_ids(const ObDMLStmt &parent_stmt,
...
int ObTransformUtils::check_has_exec_param_from_parent(ObIArray<ObParentDMLStmt> &parent_stmts,
                                                       ObDMLStmt *&stmt,
                                                       bool &has_pushed_param)
{
  int ret = OB_SUCCESS;
  has_pushed_param = false;
  ObSEArray<ObExecParamRawExpr*, 4> parent_exec_params;
  ObSEArray<ObExecParamRawExpr*, 4> used_exec_params;
  //collect the pushed-down exec params from the ancestor stmt
  for (int64_t i = 0; OB_SUCC(ret) && i < parent_stmts.count(); ++i) {
    ObDMLStmt *parent_stmt = parent_stmts.at(i).stmt_;
    if (OB_ISNULL(parent_stmt)) {
      ret = OB_ERR_UNEXPECTED;
    } else {
      //check subquery exprs
      for (int64_t j = 0; OB_SUCC(ret) && j < parent_stmt->get_subquery_expr_size(); ++j) {
        ObQueryRefRawExpr *subquery_expr = NULL;
        if (OB_ISNULL(subquery_expr = parent_stmt->get_subquery_exprs().at(j))) {
          ret = OB_ERR_UNEXPECTED;
        } else if (OB_FAIL(append(parent_exec_params, subquery_expr->get_exec_params()))) {
        }
      }
      //check lateral join table
      for (int64_t j = 0; OB_SUCC(ret) && j < parent_stmt->get_table_size(); ++j) {
        TableItem *table_item = parent_stmt->get_table_item(j);
        if (OB_ISNULL(table_item)) {
          ret = OB_ERR_UNEXPECTED;
        } else if (table_item->is_lateral_table() && 
                   OB_FAIL(append(parent_exec_params, table_item->exec_params_))) {
        }
      }
    }
  }
  if (OB_FAIL(ret)) {
  } else if (parent_exec_params.count() == 0) {
    //do nothing, no exec param pushed
  } else if (!stmt->is_select_stmt()) {
    //do nothing
  } else if (OB_FAIL(ObTransformUtils::get_exec_params(parent_exec_params, 
                                                       static_cast<ObSelectStmt*>(stmt), 
                                                       used_exec_params))) {
  } else if (!used_exec_params.empty()) {
    has_pushed_param = true;
  }
  return ret;
}
 ...
 */
int ObTransformUtils::get_parent_stmt(const ObSelectStmt *stmt,
                                      const hash::ObHashMap<uint64_t, ObParentDMLStmt, common::hash::NoPthreadDefendMode> &parent_map,
                                      uint64_t &table_id,
...
int ObTransformUtils::check_ref_fake_cte_table(ObDMLStmt *stmt, bool &ref_fake_cte_table)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(stmt)) {
    ret = OB_ERR_UNEXPECTED;
  } else {
    for (int64_t i = 0; OB_SUCC(ret) && !ref_fake_cte_table && i < stmt->get_table_size(); ++i) {
      const TableItem *table_item = NULL;
      if (OB_ISNULL(table_item = stmt->get_table_item(i))) {
        ret = OB_ERR_UNEXPECTED;
      } else if (table_item->is_fake_cte_table()) {
        ref_fake_cte_table = true;
      }
    }
  }
  if (OB_SUCC(ret) && !ref_fake_cte_table) {
    // get all child stmts
    ObSEArray<ObSelectStmt *, 4> child_stmts;
    if (OB_FAIL(stmt->get_child_stmts(child_stmts))) {
    } else {
      for (int64_t i = 0; OB_SUCC(ret) && !ref_fake_cte_table && i < child_stmts.count(); ++i) {
        if (OB_FAIL(SMART_CALL(check_ref_fake_cte_table(child_stmts.at(i), ref_fake_cte_table)))) {
        }
      }
    }
  }
