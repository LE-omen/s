...
OB_INLINE int ObRangeGenerator::generate_precise_get_range(const ObRangeNode &node)
{
  ...
  if (OB_NOT_NULL(node.and_next_) || OB_NOT_NULL(node.or_next_)) {
    ...
    for (int i = 0; OB_SUCC(ret) && !always_false && i < node.column_cnt_; ++i) {
      ...
      } else if (OB_FAIL(try_cast_value(*meta, starts[i], cmp, CO_EQ))) {
      } else if (cmp != 0) {
        always_false = true;
      } else {
        starts[i].set_collation_type(meta->column_type_.get_collation_type());
  ...
  if (range != nullptr) {
    ...
    if (OB_SUCC(ret)) {
      if (always_false) {
        ...
        if (OB_FAIL(cast_value_type(*range))) {
        } else if (need_cache) {
...
int ObRangeGenerator::merge_and_remove_ranges()
{
  ...
  if (always_true_range_ != nullptr) {
    ...
    hash::ObHashSet<ObRangeWrapper, hash::NoPthreadDefendMode> range_set;
                                                       ...
                                                       bool &can_fast_extract)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && can_fast_extract && i < pos_arr.count(); ++i) {
    ...
    if (OB_ISNULL(meta)) {
      ...
      ObObj cast_obj = objs.at(i);
      if (OB_FAIL(try_cast_value(allocator,
                                 exec_ctx,
                                 dtc_params,
                                 *meta,
                                 cur_datetime,
