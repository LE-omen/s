    ...
    common::ObIArray<ObRawExpr *> *sort_exprs)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    is_in_hit_ = false;
    if (OB_FAIL(record_in_dml_partition_info(stmt, exec_ctx, filter_exprs, is_in_hit_, table_schema))) {
    } else if (!is_in_hit_) {
                                   ...
                                   ObPartLocCalcNode *&calc_node)
{
  ...
  if (OB_ISNULL(l_expr) || OB_ISNULL(r_expr) || OB_ISNULL(partition_expr)) {
    ...
    if (!ObPreRangeGraph::can_be_extract_range(T_OP_EQ, func_expr->get_result_type(),
    ...
    if (column_id == column_expr->get_column_id()) {
      ...
      if (!ObPreRangeGraph::can_be_extract_range(T_OP_EQ, col_expr->get_result_type(),
                                                     ...
                                                     const ObIArray<ObObjectID> *part_ids) const
{
  ...
  if (OB_SUCC(ret)) {
    if (NULL != gen_col_node && NULL != se_gen_col_expr) {
      ...
      if (!gen_col_node->is_pre_range_graph_node()) {
        ret = OB_ERR_UNEXPECTED;
      } else if (OB_FAIL(calc_pre_range_graph_partition_ids(exec_ctx, tablet_mapper, params,
