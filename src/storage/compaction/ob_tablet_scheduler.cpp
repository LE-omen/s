...
/********************************************ObTabletScheduler impl******************************************/
constexpr ObMergeType ObTabletScheduler::MERGE_TYPES[];
...
void ObTabletScheduler::destroy()
{
  if (IS_INIT) {
    reset();
  }
}
void ObTabletScheduler::reset()
{
  stop();
  wait();
...
int ObTabletScheduler::reload_runtime_config()
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
...
int ObTabletScheduler::try_update_upper_trans_version_and_gc_sstable(
    ObLS &ls,
    ObCompactionScheduleIterator &iter)
{
  ...
  while (OB_SUCC(ret)) {
      if (OB_FAIL(iter.get_next_tablet(tablet_handle))) {
        ...
        // table's end_scn is used to detect concurrent table-store updates.
...
int ObTabletScheduler::schedule_all_tablets_minor()
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  ObLSService *ls_service = ::oceanbase::share::server_service<::oceanbase::storage::ObLSService>();
...
int ObTabletScheduler::check_ls_compaction_finish()
{
  int ret = OB_SUCCESS;
  bool exist = false;
...
int ObTabletScheduler::refresh_runtime_status()
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    ...
    IGNORE_RETURN runtime_status_.init_or_refresh();
  }
  return ret;
}

int ObTabletScheduler::schedule_build_bloomfilter(
...
int ObTabletScheduler::schedule_merge(const int64_t broadcast_version)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObTabletScheduler has not been inited", K(ret));
...
bool ObTabletScheduler::check_tx_table_ready(ObLS &ls, const SCN &check_scn)
{
  int ret = OB_SUCCESS;
...
int ObTabletScheduler::check_ready_for_major_merge(
...
int ObTabletScheduler::schedule_merge_dag(
    const storage::ObTablet &tablet,
    const ObMergeType merge_type,
    ...
    const ObDagId *dag_net_id /*= nullptr*/)
{
  ...
  if (OB_FAIL(check_ready_for_major_merge(tablet, merge_type))) {
    ...
    if (OB_FAIL(ObDagParamFunc::fill_param(
      tablet, merge_type, merge_snapshot_version, param))) {
...
int ObTabletScheduler::schedule_tablet_meta_merge(
    ObLS *ls,
    ObTabletHandle &tablet_handle,
  ...
  if (OB_UNLIKELY(OB_ISNULL(ls) || !tablet_handle.is_valid())) {
    ...
    if (OB_FAIL(tablet->read_medium_info_list(allocator, medium_list))) {
      ...
      if (OB_FAIL(ObAdaptiveMergePolicy::get_meta_merge_tables(
        ...
        if (OB_FAIL(ObDagParamFunc::fill_param(
          *tablet, META_MAJOR_MERGE, result.merge_version_, dag_param))) {
...
int ObTabletScheduler::fill_minor_compaction_param(
...
template <class T>
int ObTabletScheduler::schedule_tablet_minor_merge(
    ObLS *ls,
    ObTabletHandle &tablet_handle)
{
  int ret = OB_SUCCESS;
    ...
    ObTabletHandle &tablet_handle)
{
  ...
  if (OB_UNLIKELY(OB_ISNULL(ls) || !tablet_handle.is_valid() || !is_valid_merge_type(merge_type))) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(merge_type), KP(ls), K(tablet_handle));
    ...
    if (result.handle_.get_count() < minor_compact_trigger) {
      ...
      for (int64_t k = 0; OB_SUCC(ret) && k < parallel_results.count(); ++k) {
        ...
        } else if (OB_FAIL(fill_minor_compaction_param(tablet_handle, parallel_results.at(k), dag_param))) {
...
template <class T>
int ObTabletScheduler::schedule_merge_execute_dag(
    ...
    const ObGetMergeTablesResult &result)
{
  ...
  if (OB_UNLIKELY(OB_ISNULL(ls) || !tablet_handle.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(ls), K(tablet_handle));
  } else if (FALSE_IT(emergency = tablet_handle.get_obj()->get_tablet_meta().tablet_id_.is_ls_inner_tablet())) {
  } else if (result.handle_.get_count() > 1 &&
      !ObTabletScheduler::check_tx_table_ready(*ls, result.scn_range_.end_scn_)) {
    ret = OB_EAGAIN;
...
int ObTabletScheduler::schedule_minor_merge(
    ObLS *ls)
{
  int ret = OB_SUCCESS;
  ...
  } else if (FALSE_IT((void) ObLSStatusCache::check_ls_state(*ls, state))) {
  } else if (ObLSStatusCache::CAN_MERGE != state) {
...
int ObTabletScheduler::schedule_tablet_minor(
  ObLS *ls,
  ObTabletHandle tablet_handle,
  ...
  bool &need_fast_freeze_flag)
{
  ...
  ObTablet *tablet = tablet_handle.get_obj();
  ObTabletID tablet_id;
  if (OB_UNLIKELY(OB_ISNULL(ls) || OB_ISNULL(tablet))) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(ls), K(tablet_handle));
  } else if (FALSE_IT(tablet_id = tablet->get_tablet_meta().tablet_id_)) {
...
int ObTabletScheduler::schedule_ddl_tablet_merge(
    ObLS *ls,
    ObTabletHandle &tablet_handle)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(OB_ISNULL(ls) || !tablet_handle.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid arg", K(ret), KP(ls), K(tablet_handle));
    ...
    if (OB_FAIL(storage::ObDDLMergeScheduler::schedule_tablet_ddl_major_merge(ls, tablet_handle))) {
      if (OB_SIZE_OVERFLOW != ret && OB_EAGAIN != ret) {
...
int ObTabletScheduler::schedule_all_tablets_medium()
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObTabletScheduler has not been inited", K(ret));
  } else if (!runtime_status_.is_inited() && OB_FAIL(runtime_status_.init_or_refresh())) {
    if (OB_NEED_WAIT != ret) {
      LOG_WARN("failed to init runtime status", KR(ret), K_(runtime_status));
    }
  } else {
...
bool ObTabletScheduler::need_fast_medium_loop() const
{
...
int ObTabletScheduler::user_request_schedule_medium_merge(
...
int ObTabletScheduler::try_schedule_adaptive_merge(
    ObLS *ls,
    ObTabletHandle &tablet_handle,
    const ObAdaptiveMergePolicy::AdaptiveCompactionEvent &event,
    ...
    bool &create_dag)
{
  ...
  if (OB_UNLIKELY(OB_ISNULL(ls) || !tablet_handle.is_valid() || !ObAdaptiveMergePolicy::need_schedule_meta(event))) {
    ...
    if (OB_FAIL(ObAdaptiveMergePolicy::check_adaptive_merge_reason_for_event(
      ...
      if (OB_TMP_FAIL(ObTabletScheduler::schedule_tablet_meta_merge(ls, tablet_handle, create_dag))) {
